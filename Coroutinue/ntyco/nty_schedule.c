...
void nty_schedule_sched_sleepdown(nty_coroutine *co, uint64_t msecs) {
	...
	nty_coroutine_yield(co); //yeild cpu，源码忘记加上了
}
...
void nty_schedule_sched_wait(nty_coroutine *co, int fd, unsigned short events, uint64_t timeout) {
	...
	co->fd = fd;
	co->events = events;
	nty_coroutine *co_tmp = RB_INSERT(_nty_coroutine_rbtree_wait, &co->sched->waiting, co); //将当前协程插入IO等待树，但是IO事件没有来到时，永远不会被唤醒

	assert(co_tmp == NULL);

	//printf("timeout --> %"PRIu64"\n", timeout);
	if (timeout == 1) return ; //Error
	nty_schedule_sched_sleepdown(co, timeout);	//将当前协程插入sleep树，并设置睡眠时间，时间到来就被唤醒，这样这个协程序既可以被IO唤醒，也可以被睡眠唤醒
}
...
//IO　wait调度唤醒协程的核心，但是这里没有IO事件到来就会永远阻塞
...
void nty_schedule_run(void) {
	...
	while (!nty_schedule_isdone(sched)) {
		...
		// 1. expired --> sleep rbtree	睡眠的协程，可以设置让某个协程睡眠时间，时间到了，调度器自然会唤醒它的
		nty_coroutine *expired = NULL;
		while ((expired = nty_schedule_expired(sched)) != NULL) {
			nty_coroutine_resume(expired); //如果定时时间超时，这里进行resume，这里会判断有那些任务超时了，然后一起运行
		}

		// 2. ready queue，这个是nty_schedule_create
		...
		while (!TAILQ_EMPTY(&sched->ready)) {
			...
			nty_coroutine_resume(co); //ready 队列中resume
			if (co == last_co_ready) break;
		}

		// 3. wait rbtree  IO wait
		nty_schedule_epoll(sched); //这个是epoll_wait调度器，没有IO事件来临，这里永远阻塞
